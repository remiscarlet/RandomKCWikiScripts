...
__full_info__ = '''
This is the first iteration of the AmatsukazeBot/RemiBot. Random new features will be added on at random 
    ...
    if xChatMessage[0] == "!":
        ...
        if firstWord == "!topic" or firstWord == "!settopic":
            if opped:
                ...
                string.strip()
                channel = xchat.get_info("channel")
                destination.command("topic "+string)
        ...
        if firstWord in ["!herald","!heralds"]:
            if len(xChatMessage.split(" "))>1:
                ...
                if secondWord == "help":
                    say(destination, "Herald will have the bot greet you every time you join the channel with a custom message. \00304!herald <add|remove> <nick> [message]")
                    ...
                    if secondWord == "add":
                        if len(xChatMessage.split(" "))>2:
                            ...
                            if opped or isYuto(xChatNick):
                                c.execute('SELECT * FROM Heralds WHERE Nick = ? AND Network = ?', (target,network))#check
                                result = c.fetchone()
                                if result == None:
                                    c.execute('INSERT INTO Heralds VALUES (?,?,?)', (target, " ".join(message), network))
                                else:
                                    c.execute('UPDATE Heralds SET Herald=? WHERE Nick=? AND Network = ?', (" ".join(message),target,network))
                                conn.commit()
                                say(destination, "Got it! I'll say that from now on <3")
                            else:
                                say(destination,"You need to be opped to use that!")
                    elif secondWord == "remove":
                        if opped or isYuto(xChatNick):
                            c.execute('SELECT * FROM Heralds WHERE Nick = ? AND Network = ?', (target,network))#check
                            result = c.fetchone()
                            if result != None:
                                c.execute('DELETE FROM Heralds WHERE Nick = ? AND Network = ?', (target,network))
                        ...
                        say(destination,"You didn't provide enough arguments! \00304!herald <add|remove> <nick> [message]") 
            else:
                say(destination,"You didn't provide enough arguments! \00304!herald <add|remove> <nick> [message]")
        ########################
        ...
        if firstWord in ["!quote", "!quotes"]:
            if len(xChatMessage.split(" "))>2:
                ...
                if secondWord in ["add","+"]:
                    ...
                    c.execute("INSERT INTO Quotes VALUES (?,?,?,?,?,?)", (currId,timestamp,channel,nick,message,network))
                    conn.commit()
                ...
                if secondWord in ["delete","remove","del","rem","rm"] and isYuto(xChatNick):
                    ...
                    if quoteId.isdigit():
                        c.execute("SELECT * FROM Quotes WHERE Id=? AND Network=?",(quoteId,network))
                        if c.fetchone() != None:
                            c.execute("DELETE FROM Quotes WHERE Id=? AND Network=?",(quoteId,network))
                            say(destination,"Quote deleted!")
                        else:
                            say(destination,"No such quote matching that id in this network and channel!")
                    else:
                ...
                if secondWord in ["show", "tell", "see", "read"]:
                    ...
                    quoteId.replace("#","")
                    if quoteId.isdigit():
                        c.execute("SELECT * FROM Quotes WHERE Id=? AND Channel=? AND Network=?",(quoteId,channel,network))
                        result = c.fetchone()
                        if result == None:
                            say(destination,"I couldn't find a quote by that id or it doesn't belong to this channel or network!")
                        else:
                ...
                if secondWord in ["search","find","lookup"]:
                    ...
                    for term in searchTerms:
                        c.execute("SELECT Id FROM Quotes WHERE (Message LIKE ? OR Nick LIKE ?) AND Network=?",("%"+term+"%","%"+term+"%",network))
                        results = c.fetchall()
            ...
            if len(xChatMessage.split(" "))>1:
                ...
                if secondWord in ["list","all"]:
                    c.execute("SELECT Id FROM Quotes WHERE Channel=? AND Network=? ORDER BY Id DESC LIMIT 40",(channel,network))
                    results = c.fetchall()
                    print results
                    if results != []:
                        quoteMessage = ""
                ...
                if secondWord in ["random","shuffle","rand"]:
                    c.execute("SELECT Id FROM Quotes WHERE Channel=? AND Network=?",(channel,network))
                    results = c.fetchall()
                    print results
        ...
        if firstWord == "!help":
            ...
            if len(xChatMessage.split(" ")) >1:
                ...
                if secondWord in ["c", "calculator", "calc"]:
                    say(destination,"This is a smart calculator! It supports most major functions like trig, rounding(floor/round/ceil), logs, exponents, etc. \
                        You can also use variable names. Use the word 'where' in your equation to separate your eq from variables. Try typing in natural language as I account for multiple interpretations!")
        ...
        if firstWord == "!todo":
            def showTodos(destination,nick,page):
                if type(page) == int or page.isdigit():
                    page = int(page)
                    c.execute("SELECT Message,Date_Added FROM Todo WHERE Nick=? AND Network=? ORDER BY Date_Added ASC",(nick,network))
                    results = c.fetchall()
                    if results != []:
                        if len(results)>5:
                            if page == 1:
            ...
            if len(split)>1:
                ...
                if secondWord == "add":
                    if len(split)>2:
                        ...
                        c.execute("INSERT INTO Todo VALUES (?,?,?,?)", (str(time.time()),nick,message,network))
                ...
                if secondWord in ["delete","remove","rem","del"]:
                    if len(split)>2:
                        ...
                        if deleteId.isdigit():
                            ...
                            c.execute("SELECT Date_Added from Todo WHERE Nick=? AND Network=? ORDER BY Date_Added ASC", (nick,network))
                            results = c.fetchall()
                            if deleteId>0 and deleteId<=len(results):
                                timeToDelete = results[deleteId-1][0]
                                c.execute("DELETE FROM Todo WHERE Date_Added=? AND Nick=? AND Network=?",(timeToDelete,nick,network))
                                conn.commit()
        ...
        if firstWord in ["!tell","!later"]:
            if len(xChatMessage.split(" "))>2:
                ...
                c.execute("INSERT INTO Laters VALUES (?,?,?,?,?,?)",(fromNick,toNick,timeEntered,message,str(destination.get_info("channel")),network))
                conn.commit()
        ...
        if firstWord in ["!remind","!reminder"]:
            ...
            if len(split)>3:
                ...
                if timeArg == None:
                    ...
                    currTime = time.time()
                    c.execute("INSERT INTO Reminders (FromNick,ToNick,ReminderDate,DateAdded,Message,Network) VALUES (?,?,?,?,?,?)",
                                      (str(fromNick),str(toNick),float(time.mktime(timeToRemind)),float(currTime),str(msg),network))
                ...
                if split[1] in ["del","delete","rem","remove"]:
                    if split[2].isdigit():
                        c.execute("SELECT * FROM Reminders WHERE ID = ? AND Network=?",(int(split[2]),network))
                        result = c.fetchall()
                        ...
                        if result == []:
                            ...
                            c.execute("DELETE FROM Reminder WHERE ID = ? AND Network=?",(int(split[2]),network))
                            conn.commit()
...
def on_trigger(word,word_eol,userdata):
    ...
    destination = xchat.get_context()
    networkSettings = settings.getNetworkSettings(destination.get_info("network"))
    channel = destination.get_info("channel").lower()

    activeChannels = networkSettings["channels"]
    activeNicks = map(lambda x : x.lower(),networkSettings["nicks"])

    if networkSettings != None:
...
def on_join_content(word, word_eol, userdata, destination):
    ...
    xChatNickFull = word[0]
    network = destination.get_info("network")
    c.execute('SELECT * FROM heralds WHERE Nick = ? AND Network = ?', (xChatNick,network))#check
    result = c.fetchone()
    if result != None:
...
def checkForLaters(xChatNick,destination):
    ...
    for nick in nicksToLater:
        if xChatNick.lower() == nick[0].lower():
            c.execute("SELECT * FROM Laters WHERE ToNick = ? AND Network = ?",(xChatNick.lower(),network))
            messages = c.fetchall()
            for message in messages:
                ...
                xchat.get_context().command("msg "+toNick+" \00304"+fromNick+" \00307sent you a message in \00304" + channel + \
                    "\00307 on \00304"+time.strftime("%a, %d %b %Y %H:%M:%S +0500",time.gmtime(timeEntered)))
                xchat.get_context().command("msg "+toNick+" \00307"+msg)
            c.execute("DELETE FROM Laters WHERE ToNick = ? AND Network = ?",(xChatNick.lower(),network))
            conn.commit()
...
def checkForReminders(userdata):
    ...
    if HOOK != None:
        ...
        for data in results:
            destination = xchat.find_context(server=data[6])
            networkSettings = settings.getNetworkSettings(data[6])
            activeNicks = map(lambda x : x.lower(), networkSettings["nicks"])
            if destination.get_info("nick").lower() in activeNicks:
...
def checkForBotNick():
    ...
    botNickInCurrNick = reduce(lambda x,y : x or y, 
                               map(lambda nickToTest : nickToTest.lower() in nick.lower(),
                                   settings.nickList))
    botNickCorrect = reduce(lambda x,y : x or y, 
                               map(lambda nickToTest : nickToTest.lower() == nick.lower(),
                                   settings.nickList))
    if botNickInCurrNick:
        if not botNickCorrect:
            correctBotNick = filter(lambda x : x.lower() in nick.lower(), settings.nickList)[0]
            xchat.get_context().command("nick "+correctBotNick)


def pmremi(destination,text):
    destination.command("msg remi_scarlet "+text)
